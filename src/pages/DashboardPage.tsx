  ...
  return (
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        <h1 className="text-xl font-bold text-slate-800">Minhas evidências</h1>
      ...
      <div className="space-y-3">
        {items.map((ev) => (
          <Link key={ev.id} to={`/item/${ev.id}`}>
            <Card className="transition hover:ring-brand-100">
              <div className="flex items-start justify-between gap-3">
                <div className="min-w-0">
                  <h2 className="truncate font-semibold text-slate-800">{ev.title}</h2>
                  <p className="mt-0.5 text-xs text-slate-400">{formatDate(ev.fact_date)}</p>
                  {ev.description && (
                    <p className="mt-1 line-clamp-2 text-sm text-slate-500">{ev.description}</p>
                  )}
                </div>
                {ev.media && ev.media.length > 0 && (
                  <span className="shrink-0 rounded-full bg-brand-50 px-2 py-1 text-xs font-medium text-brand-600">
